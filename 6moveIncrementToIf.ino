...
void loop() 
{
  ...
  //Serial.print("currentTime: "); //so we know what we're looking at in the serial monitor
  //Serial.println(currentTime); //print the current time

  if (currentTime - prevTime > countDelay) //if 500 ms has passed
  {
    //Serial.print("prevTime before update: "); //so we know what we're looking at in the serial monitor
    //Serial.println(prevTime); //print the previous time (before updating)
    prevTime = currentTime; //update the previous time to the current time
    //Serial.print("prevTime after update: "); //so we know what we're looking at in the serial monitor
    //Serial.println(prevTime); //print the previous time (after updating)
